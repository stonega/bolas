...
export const IMAGE_DOCUMENT_WORKSPACE_VERSION = 1;
...
export function clampRect(rect, bounds = { x: 0, y: 0, width: 1, height: 1 }) {
  ...
  const right = clamp(normalized.x + normalized.width, limit.x, limit.x + limit.width);
  const bottom = clamp(normalized.y + normalized.height, limit.y, limit.y + limit.height);
...
function strokeStyle(annotation) {
  return {
    strokeColor: normalizedColor(annotation?.strokeColor, DEFAULT_ANNOTATION_STYLE.strokeColor),
    strokeWidth: normalizedSize(annotation?.strokeWidth, DEFAULT_ANNOTATION_STYLE.strokeWidth),
  };
}
...
export function normalizeAnnotation(annotation = {}, options = {}) {
  ...
  const fontWeight = Math.round(
    clamp(annotation?.fontWeight ?? DEFAULT_ANNOTATION_STYLE.fontWeight, 100, 1000),
  );
...
export function getAnnotationBounds(annotation, { includeStroke = false } = {}) {
  ...
  if (!includeStroke || annotation.type === 'text') return bounds;
...
function distanceBetween(first, second, scale = { x: 1, y: 1 }) {
  return Math.hypot((first.x - second.x) * scale.x, (first.y - second.y) * scale.y);
}
...
export function hitTestAnnotation(annotation, point, options = {}) {
  ...
  const strokeTolerance =
    tolerance + normalizedSize(annotation?.strokeWidth, 0) / 2 + sketchTolerance;
  ...
  if (annotation?.type === 'pencil') {
    ...
    if (points.length === 1) return distanceBetween(target, points[0], scale) <= strokeTolerance;

    for (let index = 1; index < points.length; index++) {
      if (distanceToSegment(target, points[index - 1], points[index], scale) <= strokeTolerance) {
        return true;
      }
    }
  ...
  if (annotation?.type === 'arrow') {
    ...
    for (let index = 1; index <= 32; index++) {
      ...
      if (distanceToSegment(target, previous, current, scale) <= strokeTolerance) return true;
  ...
  if (annotation?.type === 'text') return pointInsideRect(target, rect);

  if (annotation?.type === 'rectangle') {
    if (annotation.fillColor !== null && pointInsideRect(target, rect)) return true;
    ...
    return corners.some(
      (corner, index) =>
        distanceToSegment(target, corner, corners[(index + 1) % corners.length], scale) <=
        strokeTolerance,
    );
  }
  ...
  if (annotation?.type === 'ellipse') {
    ...
    if (annotation.fillColor !== null && normalizedDistance <= 1) return true;

    const approximateBoundaryDistance =
      Math.abs(normalizedDistance - 1) * Math.min(radiusX * scale.x, radiusY * scale.y);
...
export function hitTestAnnotations(annotations, point, options = {}) {
  ...
  for (let index = source.length - 1; index >= 0; index--) {
    if (hitTestAnnotation(source[index], point, options)) return source[index];
  }

  return null;
}

export function pickCompositionTarget({
  canvasAnnotations = [],
  canvasOptions = {},
  imageAnnotations = [],
  imageOptions = {},
  ...
  const surfaceX = finiteNumber(surfacePoint?.x, Number.NaN);
  const surfaceY = finiteNumber(surfacePoint?.y, Number.NaN);
  const canvasAnnotation =
    Number.isFinite(surfaceX) && Number.isFinite(surfaceY)
      ? hitTestAnnotations(canvasAnnotations, surfacePoint, canvasOptions)
      : null;

  if (canvasAnnotation) return { annotation: canvasAnnotation, layer: 'canvas' };

  const imageX = finiteNumber(imagePoint?.x, Number.NaN);
  const imageY = finiteNumber(imagePoint?.y, Number.NaN);
  const insideImage =
  ...
  if (insideImage) {
    return {
      annotation: hitTestAnnotations(imageAnnotations, imagePoint, imageOptions),
      layer: 'image',
    };
  }

  return { annotation: null, layer: 'canvas' };
}

export function editPanelForCompositionTarget(target = {}) {
  if (target.annotation) return 'draw';
  return target.layer === 'image' ? 'crop' : 'background';
}
...
export function hitTestResizeHandle(bounds, point, options = {}) {
  ...
  return (
    getResizeHandles(bounds).find(
      ({ point: handlePoint }) => distanceBetween(target, handlePoint, scale) <= tolerance,
    )?.handle ?? null
  );
}
...
export function mapAnnotationGeometry(annotation, pointMapper, options = {}) {
  ...
  if ('strokeWidth' in mapped) mapped.strokeWidth *= styleScale;

  if ('fontSize' in mapped) mapped.fontSize *= styleScale;
...
export function resizeBounds(bounds, handle, dx, dy, options = {}) {
  ...
  if (movesWest) left = Math.min(left + finiteNumber(dx), right - minimum);
  if (movesEast) right = Math.max(right + finiteNumber(dx), left + minimum);
  if (movesNorth) top = Math.min(top + finiteNumber(dy), bottom - minimum);
  if (movesSouth) bottom = Math.max(bottom + finiteNumber(dy), top + minimum);
  ...
  if (options.keepAspect && rect.width > 0 && rect.height > 0) {
    ...
    if ((movesWest || movesEast) && (movesNorth || movesSouth)) {
      ...
      if (candidateWidth / candidateHeight > ratio) {
        ...
        if (movesNorth) top = bottom - wantedHeight;
        else bottom = top + wantedHeight;
      } else {
        const wantedWidth = candidateHeight * ratio;
        if (movesWest) left = right - wantedWidth;
        else right = left + wantedWidth;
      }
  ...
  if (options.clamp) resized = clampRect(resized);
...
export function resizeAnnotationGeometry(annotation, oldBounds, newBounds, options = {}) {
  ...
  const scaleX = source.width > MIN_NORMALIZED_SIZE ? target.width / source.width : 1;
  const scaleY = source.height > MIN_NORMALIZED_SIZE ? target.height / source.height : 1;
  const styleScale = options.scaleStyle ? Math.sqrt(Math.abs(scaleX * scaleY)) : 1;

  return mapAnnotationGeometry(
    annotation,
    (point) => ({
      x:
        source.width > MIN_NORMALIZED_SIZE
          ? target.x + (point.x - source.x) * scaleX
          : point.x + target.x - source.x,
      y:
        source.height > MIN_NORMALIZED_SIZE
...
export function transformTextOrientation(annotation, matrix) {
  if (annotation?.type !== 'text') return normalizeAnnotation(annotation, { id: annotation?.id });
...
export function rotateAnnotation(annotation, quarterTurn = 1) {
  ...
  const matrix = clockwise ? { a: 0, b: -1, c: 1, d: 0 } : { a: 0, b: 1, c: -1, d: 0 };
  let mapped = mapAnnotationGeometry(
    annotation,
    clockwise
  ...
  if (mapped.type === 'text') mapped = transformTextOrientation(mapped, matrix);
...
export function flipAnnotation(annotation, axis = 'horizontal') {
  ...
  const matrix =
    normalizedAxis === 'horizontal' ? { a: -1, b: 0, c: 0, d: 1 } : { a: 1, b: 0, c: 0, d: -1 };
  let mapped = mapAnnotationGeometry(
    annotation,
...
function normalizeWorkspaceState(value, fallback = {}) {
  if (!value || typeof value !== 'object' || Array.isArray(value))
    throw new TypeError('Document history states must be objects.');

  const annotations = Array.isArray(value.annotations)
    ? value.annotations.map((annotation) => normalizeAnnotation(annotation))
    : [];
  const transforms = Array.isArray(value.transforms)
    ? value.transforms.map((transform) => normalizeTransform(transform))
    : [];
  const requestedSelectionId = value.selectionId == null ? null : String(value.selectionId);
  const selectionId = annotations.some((annotation) => annotation.id === requestedSelectionId)
    ? requestedSelectionId
    : null;

  return {
    width: positiveDimension(value.width, positiveDimension(fallback.width)),
    height: positiveDimension(value.height, positiveDimension(fallback.height)),
    annotations,
    transforms,
    selectionId,
  };
...
export class ImageDocument {
  ...
  static fromWorkspaceSnapshot(value = {}) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
      throw new TypeError('The document workspace snapshot must be an object.');
    if (Number(value.version) !== IMAGE_DOCUMENT_WORKSPACE_VERSION)
      throw new Error('The document workspace version is not supported.');

    const state = normalizeWorkspaceState(value.state, {
      height: value.originalHeight,
      width: value.originalWidth,
    });
    const historyLimit = Math.min(
      MAX_HISTORY_ENTRIES,
      Math.max(1, Math.round(finiteNumber(value.historyLimit, MAX_HISTORY_ENTRIES))),
    );
    const document = new ImageDocument({
      ...state,
      historyLimit,
      originalHeight: positiveDimension(value.originalHeight, state.height),
      originalWidth: positiveDimension(value.originalWidth, state.width),
    });

    document._restoreState(state);
    document._undoStack = (Array.isArray(value.undo) ? value.undo : [])
      .slice(-historyLimit)
      .map((entry) => normalizeWorkspaceState(entry, state));
    document._redoStack = (Array.isArray(value.redo) ? value.redo : [])
      .slice(-historyLimit)
      .map((entry) => normalizeWorkspaceState(entry, state));
    document.markSaved();
    return document;
  ...
  addAnnotation(annotation) {
    ...
    return this._mutate(() => {
      ...
      if (this.annotations.some((item) => item.id === added.id)) added.id = createAnnotationId();
  ...
  updateAnnotation(id, patchOrUpdater) {
    ...
    return this._mutate(() => {
      ...
      const update =
        typeof patchOrUpdater === 'function' ? patchOrUpdater(deepClone(current)) : patchOrUpdater;
      ...
      for (const key of ['rect', 'start', 'end', 'bend']) {
        if (update[key]) merged[key] = { ...current[key], ...update[key] };
      }
  ...
  deleteAnnotation(id = this.selectionId) {
    ...
    return this._mutate(() => {
      ...
      if (this.selectionId === annotationId) this.selectionId = null;
      return removed;
    });
  }
  ...
  duplicateAnnotation(id = this.selectionId, offset = { x: 0.02, y: 0.02 }) {
    ...
    return this._mutate(() => {
      const duplicate = moveAnnotationGeometry(
        ...
        finiteNumber(offset?.x, 0.02),
        finiteNumber(offset?.y, 0.02),
        { clamp: true },
      );
  ...
  moveAnnotation(id, dx, dy, options = {}) {
    ...
    return this._mutate(() => {
      this.annotations[index] = moveAnnotationGeometry(this.annotations[index], dx, dy, options);
      return this.annotations[index];
    });
  }
  ...
  resizeAnnotation(id, handle, dx, dy, options = {}) {
    ...
    return this._mutate(() => {
      ...
      this.annotations[index] = resizeAnnotationGeometry(annotation, oldBounds, newBounds, options);
      return this.annotations[index];
    });
  }
  ...
  rotate(quarterTurns = 1) {
    ...
    if (requestedTurns === 0 || requestedTurns % 4 === 0) return false;
  ...
  clearHistory() {
    if (this._transaction) this.cancelTransaction();
    this._undoStack = [];
    this._redoStack = [];
  }
  ...
  toWorkspaceSnapshot() {
    if (this._transaction) throw new Error('Finish the current edit before saving the workspace.');

    return deepClone({
      version: IMAGE_DOCUMENT_WORKSPACE_VERSION,
      historyLimit: this.historyLimit,
      originalWidth: this.originalWidth,
      originalHeight: this.originalHeight,
      state: this._captureState(),
      undo: this._undoStack.slice(-this.historyLimit),
      redo: this._redoStack.slice(-this.historyLimit),
    });
  }
}
