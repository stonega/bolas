...
import { DEFAULT_ANNOTATION_STYLE, HAND_DRAWN_ROUGHNESS } from './document.js';
...
function pathsReferToSameFile(leftPath, rightPath) {
  ...
  if (canonicalPath(leftPath) === canonicalPath(rightPath)) return true;
...
function buildLoadedImage(path, animation, rawPixbuf) {
  ...
  const isVector = Boolean(format?.is_scalable?.()) || metadata.mimeType === 'image/svg+xml';
...
export function loadImageSource(path) {
  ...
  if (!rawPixbuf) throw new Error(`The image decoder did not provide a frame for ${localPath}.`);

  return buildLoadedImage(localPath, animation, rawPixbuf);
}

/** Decode the normalized PNG embedded in a Bolas workspace. */
export function loadImageSourceBytes(value, metadata = {}) {
  const bytes = value instanceof GLib.Bytes ? value : new GLib.Bytes(value);
  const stream = Gio.MemoryInputStream.new_from_bytes(bytes);

  try {
    const animation = GdkPixbuf.PixbufAnimation.new_from_stream(stream, null);
    const rawPixbuf = animation.get_static_image();
    if (!rawPixbuf) throw new Error('The workspace image decoder did not provide a frame.');
    ...
    return {
      ...
      formatName: 'png',
      displayName: String(metadata.displayName ?? 'Image'),
      fileSize: bytes.get_size(),
      isAnimated: false,
      isVector: false,
      isStatic: true,
    };
...
/** Encode an editor source pixbuf as a portable, orientation-normalized PNG. */
export function encodePixbufPng(pixbuf) {
  if (!pixbuf) throw new Error('A source pixbuf is required.');
  const [success, bytes] = pixbuf.save_to_bufferv('png', [], []);
  if (!success || !bytes?.length)
    throw new Error('The workspace source image could not be encoded.');
...
function transformType(transform) {
  ...
  if (transform?.rect) return 'crop';
  if (transform?.quarterTurns != null || transform?.direction) return 'rotate';
  if (transform?.axis || transform?.horizontal || transform?.vertical) return 'flip';
...
function cropPixbuf(pixbuf, transform) {
  ...
  return pixbuf.new_subpixbuf(left, top, cropWidth, cropHeight).copy();
}

function quarterTurnsForTransform(transform) {
  if (transform?.quarterTurns != null) return Math.trunc(finiteNumber(transform.quarterTurns));
...
function flipPixbuf(pixbuf, transform) {
  ...
  if (horizontal) output = output.flip(true);
  if (vertical) output = output.flip(false);
...
export function applyImageTransforms(pixbuf, transforms = []) {
  ...
  if (!Array.isArray(transforms)) throw new Error('Image transforms must be an array.');
...
function documentTransforms(document) {
  if (typeof document?.getTransforms === 'function') return document.getTransforms();

  return document?.transforms ?? [];
}

function documentAnnotations(document) {
  if (typeof document?.getAnnotations === 'function') return document.getAnnotations();

  return document?.annotations ?? [];
}

function cropSurfaceMetrics(width, height, transform) {
  const rect = transform?.rect;
  const rectWidth = finiteNumber(rect?.width, NaN);
  const rectHeight = finiteNumber(rect?.height, NaN);
  const x = finiteNumber(rect?.x, NaN);
  ...
  return {
    height: outputHeight,
    left: clamp(Math.floor(x * width), 0, width - outputWidth),
    top: clamp(Math.floor(y * height), 0, height - outputHeight),
    width: outputWidth,
  };
}

function canvasTransformStages(width, height, transforms = []) {
  const stages = [];
  ...
  for (const transform of transforms) {
    ...
    const before = { height: currentHeight, width: currentWidth };
    let details = null;

    if (type === 'crop') {
      details = cropSurfaceMetrics(currentWidth, currentHeight, transform);
      currentWidth = details.width;
      currentHeight = details.height;
    } else if (type === 'rotate') {
      const quarterTurns = ((quarterTurnsForTransform(transform) % 4) + 4) % 4;
      details = { quarterTurns };
      if (quarterTurns % 2 === 1) [currentWidth, currentHeight] = [currentHeight, currentWidth];
    } else if (type === 'flip') {
      const axis = String(transform?.axis ?? '').toLowerCase();
      ...
      throw new Error(`Unsupported image transform: ${type || '(missing type)'}.`);
    }

    stages.push({
      after: { height: currentHeight, width: currentWidth },
      before,
      details,
      type,
    });
...
function forwardCanvasPoint(point, stage) {
  const { before, details, type } = stage;

  if (type === 'crop') return { x: point.x - details.left, y: point.y - details.top };
  if (type === 'flip')
    return details.axis === 'horizontal'
      ? { x: before.width - point.x, y: point.y }
      : { x: point.x, y: before.height - point.y };
  if (details.quarterTurns === 1) return { x: before.height - point.y, y: point.x };
  if (details.quarterTurns === 2) return { x: before.width - point.x, y: before.height - point.y };
  if (details.quarterTurns === 3) return { x: point.y, y: before.width - point.x };
  return point;
}

function inverseCanvasPoint(point, stage) {
  const { before, details, type } = stage;
  ...
  if (type === 'flip')
    return details.axis === 'horizontal'
      ? { x: before.width - point.x, y: point.y }
      : { x: point.x, y: before.height - point.y };
  if (details.quarterTurns === 1) return { x: point.y, y: before.height - point.x };
  if (details.quarterTurns === 2) return { x: before.width - point.x, y: before.height - point.y };
  if (details.quarterTurns === 3) return { x: before.width - point.y, y: point.x };
  return point;
}

export function compositionCanvasSize(options = {}, transforms = []) {
  const normalized = normalizeShareOptions(options);
  const base = shareCanvasSize(normalized.ratioId);
  const stages = canvasTransformStages(base.width, base.height, transforms);
  return stages.at(-1)?.after ?? base;
}

export function editorSurfaceDimensions({
  ...
  if (backgroundEnabled) return compositionCanvasSize(canvasOptions, canvasTransforms);

  return {
    height: Math.max(1, finiteNumber(renderedHeight ?? imageHeight, 1)),
    width: Math.max(1, finiteNumber(renderedWidth ?? imageWidth, 1)),
  };
}

export function fittedSurfaceGeometry(
  surfaceWidth,
  surfaceHeight,
  viewportWidth,
  viewportHeight,
  ...
  const width = Math.max(1, finiteNumber(surfaceWidth, 1));
  const height = Math.max(1, finiteNumber(surfaceHeight, 1));
  const canvasWidth = Math.max(1, finiteNumber(viewportWidth, 1));
  const canvasHeight = Math.max(1, finiteNumber(viewportHeight, 1));
  ...
  const scale = fitScale * Math.max(0.0001, finiteNumber(options.zoom, 1));
  ...
  return {
    ...
    scale,
    width,
    x: (canvasWidth - width * scale) / 2 + finiteNumber(options.panX),
    y: (canvasHeight - height * scale) / 2 + finiteNumber(options.panY),
  };
}

export function imagePointToComposition(
  point,
  imageWidth,
  imageHeight,
  options = {},
  ...
  const normalized = normalizeShareOptions(options);
  const base = shareCanvasSize(normalized.ratioId);
  const layout = shareImageLayout(imageWidth, imageHeight, base.width, base.height, normalized);
  const stages = canvasTransformStages(base.width, base.height, transforms);
  let mapped = {
    x: layout.x + finiteNumber(point?.x) * layout.width,
    y: layout.y + finiteNumber(point?.y) * layout.height,
  };
  for (const stage of stages) mapped = forwardCanvasPoint(mapped, stage);
  const output = stages.at(-1)?.after ?? base;
  return { x: mapped.x / output.width, y: mapped.y / output.height };
}
...
export function compositionPointToImage(
  ...
  const output = stages.at(-1)?.after ?? base;
  let mapped = {
    x: finiteNumber(point?.x) * output.width,
    y: finiteNumber(point?.y) * output.height,
  };
  for (let index = stages.length - 1; index >= 0; index--)
    mapped = inverseCanvasPoint(mapped, stages[index]);
  return {
    x: (mapped.x - layout.x) / layout.width,
    y: (mapped.y - layout.y) / layout.height,
  };
}

export function imageBoundsInComposition(imageWidth, imageHeight, options = {}, transforms = []) {
  const points = [
    { x: 0, y: 0 },
    { x: 1, y: 0 },
    { x: 0, y: 1 },
    { x: 1, y: 1 },
  ].map((point) => imagePointToComposition(point, imageWidth, imageHeight, options, transforms));
...
function parseColor(value, fallback = '#000000') {
  ...
  if (value && typeof value === 'object') {
    const scale = [value.red, value.green, value.blue, value.alpha].some(
      (component) => finiteNumber(component) > 1,
    )
      ? 255
      : 1;
  ...
  if (rgba.parse(String(value ?? fallback))) return rgba;
  if (value !== fallback && rgba.parse(fallback)) return rgba;
...
function setSourceColor(cr, value, opacity = 1, fallback = '#000000') {
  ...
  cr.setSourceRGBA(color.red, color.green, color.blue, clamp(color.alpha * opacity, 0, 1));
}

function annotationStrokeWidth(annotation, minimumDimension) {
  const normalized = Math.max(0, finiteNumber(annotation?.strokeWidth, DEFAULT_STROKE_WIDTH));
...
function sketchRandom(annotation, salt) {
  const identity = annotation?.id ? String(annotation.id) : JSON.stringify(annotation ?? {});
...
function appendRoughLinePass(cr, start, end, random, gesture, overlay) {
  ...
  const jitterScale = overlay ? ROUGH_RETRACE_JITTER_SCALE : ROUGH_PRIMARY_JITTER_SCALE;
  const jitter = gesture.boundedOffset * gesture.gain * jitterScale;

  cr.moveTo(start.x, start.y);
  cr.curveTo(
    start.x +
      gesture.deltaX * gesture.divergePoint +
      gesture.displacementX +
      randomOffset(random, jitter),
    start.y +
      gesture.deltaY * gesture.divergePoint +
      gesture.displacementY +
      randomOffset(random, jitter),
    start.x +
      gesture.deltaX * gesture.divergePoint * 2 +
      gesture.displacementX +
      randomOffset(random, jitter),
    start.y +
      gesture.deltaY * gesture.divergePoint * 2 +
      gesture.displacementY +
      randomOffset(random, jitter),
    end.x,
    end.y,
...
function createRoughLineGesture(start, end, random, maxRandomnessOffset) {
  const deltaX = end.x - start.x;
  const deltaY = end.y - start.y;
  const length = Math.hypot(deltaX, deltaY);
  const gain = roughnessGain(length);
  const boundedOffset =
    maxRandomnessOffset * maxRandomnessOffset * 100 > length * length
      ? length / 10
      : maxRandomnessOffset;
  const lengthBowingGain = clamp((length / 200) * gain, 0.35, 1);
  const bowDirection = random() < 0.5 ? -1 : 1;
  const bowAmount =
    ROUGH_BOWING * boundedOffset * lengthBowingGain * randomBetween(random, 0.55, 1) * bowDirection;
...
function appendRoughDoubleLine(cr, start, end, random, maxRandomnessOffset) {
  const gesture = createRoughLineGesture(start, end, random, maxRandomnessOffset);

  appendRoughLinePass(cr, start, end, random, gesture, false);
  appendRoughLinePass(cr, start, end, random, gesture, true);
}

function appendRoughQuadraticPass(cr, start, control, end, random, gesture, overlay) {
  if (gesture.length <= Number.EPSILON) {
    cr.moveTo(start.x, start.y);
    cr.lineTo(end.x, end.y);
    return;
...
function appendRoughDoubleQuadratic(cr, start, control, end, random, maxRandomnessOffset) {
  const gesture = createRoughLineGesture(start, end, random, maxRandomnessOffset);

  appendRoughQuadraticPass(cr, start, control, end, random, gesture, false);
  appendRoughQuadraticPass(cr, start, control, end, random, gesture, true);
}
...
function ellipseIncrement(rect) {
  ...
  const stepCount = Math.ceil(
    Math.max(ROUGH_CURVE_STEP_COUNT, (ROUGH_CURVE_STEP_COUNT / Math.sqrt(200)) * perimeterScale),
  );

  return (Math.PI * 2) / stepCount;
}

function roughEllipsePoints(rect, random, increment, pointOffset, overlap) {
  const radiusX = Math.abs(rect.width) / 2;
  const radiusY = Math.abs(rect.height) / 2;
  ...
  points.push({
    x:
      centerX +
      randomOffset(random, pointOffset) +
  ...
  for (let angle = radialStart; angle < endAngle; angle += increment) {
    points.push({
      x: centerX + randomOffset(random, pointOffset) + radiusX * Math.cos(angle),
      y: centerY + randomOffset(random, pointOffset) + radiusY * Math.sin(angle),
    });
  }
...
function appendRoughEllipsePass(cr, rect, random, maxRandomnessOffset, passScale, overlap) {
  const pointOffset = Math.max(0.5, maxRandomnessOffset * 0.65 * passScale);
  appendRoughCurve(
    cr,
    roughEllipsePoints(rect, random, ellipseIncrement(rect), pointOffset, overlap),
  );
}
...
function appendRoughDoubleEllipse(cr, rect, random, maxRandomnessOffset) {
  ...
  appendRoughCurve(
    cr,
    points.map((point) => ({
      x: point.x + randomOffset(random, maxRandomnessOffset * ROUGH_RETRACE_JITTER_SCALE),
      y: point.y + randomOffset(random, maxRandomnessOffset * ROUGH_RETRACE_JITTER_SCALE),
    })),
  );
}

function sketchMaxRandomnessOffset(minimumDimension) {
  return Math.max(ROUGH_MIN_RANDOMNESS_OFFSET, minimumDimension * HAND_DRAWN_ROUGHNESS);
}

function strokeRoughLines(cr, annotation, minimumDimension, salt, segments) {
...
function paintRoughPolygon(cr, annotation, minimumDimension, salt, points) {
  const maxRandomnessOffset = sketchMaxRandomnessOffset(minimumDimension);
  ...
  if (fillColor) {
    ...
    setSourceColor(cr, fillColor, annotationOpacity(annotation), 'transparent');
    cr.fill();
  }

  if (annotation?.strokeColor === null || annotation?.strokeColor === 'transparent') return;
...
function paintRoughEllipse(cr, annotation, minimumDimension, rect) {
  ...
  if (fillColor) {
    ...
    cr.newPath();
    appendRoughEllipsePass(cr, rect, random, maxRandomnessOffset, 1, overlap);
    cr.closePath();
    setSourceColor(cr, fillColor, annotationOpacity(annotation), 'transparent');
    cr.fill();
  }
...
function drawLine(cr, annotation, width, height, minimumDimension) {
  ...
  strokeRoughLines(cr, annotation, minimumDimension, 'line', [[start, end]]);
}
...
function drawArrow(cr, annotation, width, height, minimumDimension) {
  ...
  const bend = annotation?.bend ? normalizedPoint(annotation.bend, width, height) : chordMidpoint;
  const control = {
  ...
  if (length < 0.5) {
    configureStroke(cr, annotation, minimumDimension);
    cr.arc(end.x, end.y, annotationStrokeWidth(annotation, minimumDimension) / 2, 0, Math.PI * 2);
    cr.fill();
    return;
  }

  const requestedHeadLength = finiteNumber(annotation?.headSize, 0) * minimumDimension;
  ...
  cr.newPath();
  appendRoughDoubleQuadratic(cr, start, control, end, random, maxRandomnessOffset);
...
function drawRectangle(cr, annotation, width, height, minimumDimension) {
  const rect = normalizedRect(annotation?.rect ?? annotation?.bounds ?? annotation, width, height);
  ...
  paintRoughPolygon(cr, annotation, minimumDimension, 'rectangle', points);
}
...
function drawText(cr, annotation, width, height, minimumDimension) {
  ...
  description.set_family(String(annotation?.fontFamily ?? DEFAULT_ANNOTATION_STYLE.fontFamily));
...
function drawAnnotation(cr, annotation, width, height) {
  ...
  if (annotationOpacity(annotation) <= 0) return;
...
export function renderDocumentToSurface(sourcePixbuf, document) {
  if (!sourcePixbuf) throw new Error('A source pixbuf is required.');

  const transformed = applyImageTransforms(sourcePixbuf, documentTransforms(document));
  ...
  try {
    ...
    try {
      ...
      for (const annotation of annotations) drawAnnotation(cr, annotation, width, height);
    } finally {
      cr.$dispose();
    }
...
function surfaceDimensions(surface) {
  return {
    height: surface?.getHeight?.() ?? surface?.get_height?.() ?? 0,
    width: surface?.getWidth?.() ?? surface?.get_width?.() ?? 0,
  };
}

function transformSurface(sourceSurface, transform) {
  const before = surfaceDimensions(sourceSurface);
  const [stage] = canvasTransformStages(before.width, before.height, [transform]);
  const output = new Cairo.ImageSurface(
    Cairo.Format.ARGB32,
    Math.max(1, Math.round(stage.after.width)),
    Math.max(1, Math.round(stage.after.height)),
  );
  const cr = new Cairo.Context(output);

  try {
    cr.setOperator(Cairo.Operator.SOURCE);
    if (stage.type === 'crop') {
      cr.setSourceSurface(sourceSurface, -stage.details.left, -stage.details.top);
    } else if (stage.type === 'flip') {
      if (stage.details.axis === 'horizontal') {
        cr.translate(before.width, 0);
        cr.scale(-1, 1);
      } else {
        cr.translate(0, before.height);
        cr.scale(1, -1);
      }
      cr.setSourceSurface(sourceSurface, 0, 0);
    } else if (stage.details.quarterTurns === 1) {
      cr.translate(before.height, 0);
      cr.rotate(Math.PI / 2);
      cr.setSourceSurface(sourceSurface, 0, 0);
    } else if (stage.details.quarterTurns === 2) {
      cr.translate(before.width, before.height);
      cr.rotate(Math.PI);
      cr.setSourceSurface(sourceSurface, 0, 0);
    } else if (stage.details.quarterTurns === 3) {
      cr.translate(0, before.width);
      cr.rotate(-Math.PI / 2);
      cr.setSourceSurface(sourceSurface, 0, 0);
    } else {
      cr.setSourceSurface(sourceSurface, 0, 0);
...
function applySurfaceTransforms(sourceSurface, transforms = []) {
  let current = sourceSurface;

  for (const transform of transforms) {
    const next = transformSurface(current, transform);
    if (current !== sourceSurface) current.finish();
    current = next;
  }
  return current;
}
...
/**
 * Render the editable image layer into a share canvas, apply canvas-level
 * transforms, and finally paint canvas annotations. The two documents stay
 * independent so selecting the imported image never flattens earlier work.
 */
export function renderCompositionToSurface(
  sourcePixbuf,
  imageDocument,
  canvasDocument,
  options = {},
  ...
  if (!sourcePixbuf) throw new Error('A source pixbuf is required.');
  const normalized = normalizeShareOptions(options);
  const canvas = shareCanvasSize(normalized.ratioId);
  const imageSurface = renderDocumentToSurface(sourcePixbuf, imageDocument);
  const baseSurface = new Cairo.ImageSurface(Cairo.Format.ARGB32, canvas.width, canvas.height);

  try {
    const cr = new Cairo.Context(baseSurface);
    try {
      cr.setAntialias(Cairo.Antialias.BEST);
      paintShareSurface(cr, imageSurface, canvas.width, canvas.height, {
        ...normalized,
        sourceHasTransparency: Boolean(options.sourceHasTransparency),
      });
    } finally {
      cr.$dispose();
    }
  ...
  const transformed = applySurfaceTransforms(baseSurface, documentTransforms(canvasDocument));
  if (transformed !== baseSurface) baseSurface.finish();
  const annotations = documentAnnotations(canvasDocument);

  try {
    if (!Array.isArray(annotations)) throw new Error('Canvas annotations must be an array.');
    const { width, height } = surfaceDimensions(transformed);
    const cr = new Cairo.Context(transformed);
    try {
      cr.setOperator(Cairo.Operator.OVER);
      cr.setAntialias(Cairo.Antialias.BEST);
      for (const annotation of annotations) drawAnnotation(cr, annotation, width, height);
 ...
 */
export function exportDocumentPng(sourcePixbuf, document, targetPath, { sourcePath } = {}) {
  ...
  let fileSize = null;
  try {
    fileSize = Gio.File.new_for_path(outputPath)
...
export function exportCompositionPng(
  ...
  const surface = renderCompositionToSurface(sourcePixbuf, imageDocument, canvasDocument, options);
  const { width, height } = surfaceDimensions(surface);

  try {
    surface.writeToPNG(tempPath);
    if (GLib.chmod(tempPath, 0o600) !== 0)
      throw new Error('Could not secure the temporary composed image.');
    atomicMove(tempPath, outputPath);
    if (GLib.chmod(outputPath, 0o600) !== 0)
      throw new Error('Could not secure the exported composed image.');
  } finally {
    surface.finish();
    if (GLib.file_test(tempPath, GLib.FileTest.EXISTS)) GLib.unlink(tempPath);
  }

  return { height, mimeType: PNG_MIME_TYPE, path: outputPath, width };
}

export function defaultEditedImageDirectory(appId = 'io.github.stonega.Bolas') {
  return GLib.build_filenamev([GLib.get_user_data_dir(), appId, 'edited-images']);
}

function safeOutputStem(sourcePath) {
  const basename = sourcePath ? GLib.path_get_basename(String(sourcePath)) : 'image';
...
export function createEditedImagePath(sourcePath, options = {}) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const path = GLib.build_filenamev([directory, `${stem}-${suffix}-${timestamp}-${uuid}.png`]);
...
/**
 * Flatten an edited document into Bolas's durable share-output directory.
 */
export function saveDocumentForShare(sourcePixbuf, document, sourcePath, options = {}) {
  ...
  const result = exportDocumentPng(sourcePixbuf, document, path, { sourcePath });
...
/**
 * Flatten an edited composition into Bolas's durable output directory.
 */
export function saveCompositionForShare(
  sourcePixbuf,
  imageDocument,
  canvasDocument,
  compositionOptions,
  ...
  const path = createEditedImagePath(sourcePath, { ...options, suffix: options.suffix ?? 'share' });
  const result = exportCompositionPng(
    sourcePixbuf,
    imageDocument,
    canvasDocument,
    compositionOptions,
